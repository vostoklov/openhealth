...
- **`.rail-nav` / `.rail-btn`** (V2) — правый навбар: круглые кнопки `flex:0 0 auto; aspect-ratio:1`; `gap` гарантирует зазор даже при нехватке высоты; навигация через `scrollIntoView` + scroll-spy (клик делегирован на контейнер — переживает перестройку при смене персоны).
- **`.oh-kcard` / `.oh-kgrid` / `.oh-kcat`** (слой знаний, раздел «Девайсы и источники») — карточка девайса/источника: `.oh-kcard__top` (имя + бейдж доказательности), measures/meta, `.oh-kcard__actions` (ссылка-источник + `.oh-kverify` «перепроверить»). Группируются по категориям (`.oh-kcat`) в адаптивную сетку.
- **`.oh-ev--{high,mid,low}`** — бейдж доказательности (честная шкала high/medium/low ↔ C1-C5): зелёный/янтарный/красный, полупрозрачный фон работает и на тёмной, и на светлой теме. Тот же бейдж — в попапе «?» рядом с короткими видео к метрике. Стили knowledge/бейджей self-inject из `oh-provenance.js`, тематизируются CSS-переменными.
- **Навигация группами** строится из `OH.nav.groups()` (V1 сайдбар, V2 `.rail-nav`); persona-пикер в настройках обоих скинов переупорядочивает её через `OH.personaGroups` (opt-in, по умолчанию навигация не меняется).
